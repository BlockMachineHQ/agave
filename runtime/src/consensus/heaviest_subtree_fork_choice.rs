...
use {
    crate::{
        bank::Bank, bank_forks::BankForks, consensus::tree_diff::TreeDiff,
    ...
    std::{
        borrow::Borrow,
        cmp::Ordering,
        collections::{BTreeMap, BTreeSet, HashMap, VecDeque, btree_set::Iter, hash_map::Entry},
        sync::{Arc, RwLock},
...
impl HeaviestSubtreeForkChoice {
    ...
    pub fn add_votes<'a, 'b>(
        ...
        self.add_votes_with_stake_lookup(pubkey_votes, epoch_schedule, |epoch, pubkey| {
            epoch_stakes
                .get(&epoch)
                .map(|stakes| stakes.vote_account_stake(pubkey))
                .unwrap_or(0)
        })
    }

    /// Add votes using stake at each vote's epoch. Missing stake must resolve to zero.
    /// Epoch selection, duplicate filtering, and update ordering remain in fork choice.
    pub fn add_votes_with_stake_lookup<'b>(
        &mut self,
        pubkey_votes: impl Iterator<Item = impl Borrow<(Pubkey, SlotHashKey)> + 'b>,
        epoch_schedule: &EpochSchedule,
        stake_lookup: impl Fn(Epoch, &Pubkey) -> u64,
    ) -> SlotHashKey {
        // Generate the set of updates
        let update_operations_batch = self.generate_update_operations_with_stake_lookup(
            pubkey_votes,
            epoch_schedule,
            stake_lookup,
        );
    ...
    fn generate_update_operations<'a, 'b>(
        ...
        self.generate_update_operations_with_stake_lookup(
            pubkey_votes,
            epoch_schedule,
            |epoch, pubkey| {
                epoch_stakes
    ...
    fn generate_update_operations_with_stake_lookup<'b>(
        &mut self,
        pubkey_votes: impl Iterator<Item = impl Borrow<(Pubkey, SlotHashKey)> + 'b>,
        epoch_schedule: &EpochSchedule,
        stake_lookup: impl Fn(Epoch, &Pubkey) -> u64,
            ...
            match pubkey_latest_vote.as_mut() {
                ...
                _ => {
                        ...
                        self.latest_votes.insert(*pubkey, *new_vote_slot_hash)
                    {
                        ...
                        let stake_update = stake_lookup(epoch, pubkey);
    ...
    pub fn mark_fork_invalid_candidate(&mut self, invalid_slot_hash_key: &SlotHashKey) {
    ...
    pub fn mark_fork_valid_candidate(
        &mut self,
    ...
    fn test_new_from_frozen_banks() {
        ...
        */
        ...
        for (slot, parent) in [(1, 0), (2, 1), (3, 1), (4, 2)] {
            let parent = bank_forks.read().unwrap().get(parent).unwrap();
            let bank = Bank::new_from_parent(parent, Default::default(), slot);
            bank.freeze();
